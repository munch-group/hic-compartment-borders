...
The new variables `grw_sigma` was drawn from a Half-Normal distribution and given to the `logit_w` that models the GRW with `pymc.GaussianRandomWalk`

#### Non-centered GRW 

There are different opinions on when to reparameterize a centered model into a non-centered model [ref], but in my case, PyMC suggested a reparameterization.
